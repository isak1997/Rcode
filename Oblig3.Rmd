...
h <- function(x, alfa=3.7, lambda=1) alfa*log(lambda)+alfa*log(x)+log(log(x))-lambda*x 
  
h_d <- function(x, alfa=3.7, lambda=1) alfa/x + 1/(x*log(x)) - lambda

h_dd <-function(x, alfa=3.7, lambda=1) return(-(alfa*(log(x))^2 + log(x)+1)/(x^2*(log(x))^2))
...
I = exp(h(x0)) * sqrt(2*pi)*sigma*(pnorm((Inf-x0)/sigma) - pnorm((1-x0)/sigma))
...
I = exp(h(x0)) * sqrt(2*pi)*sigma*(pnorm((Inf-x0)/sigma) - pnorm((0-x0)/sigma))
